...
def find_tree_files(
    tree_sha: str, gitdir: pathlib.Path, previous: str = ""
    ...
    for entry in tree_entries:
        ...
        if path.is_dir():
            previous += str(path) + os.sep
        if pointer_type == "tree":
            tree_files += find_tree_files(entry[1], gitdir, previous)
        else:
            tree_files.append((entry[1], previous + str(path)))
    return tree_files
